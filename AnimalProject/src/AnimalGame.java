...
public class AnimalGame {
	...
	public static boolean isDenisThere(int row, int col, char[][] arr, Animal[][] animalBoard) {//check if denis is on the current spot
	...
	public static boolean isPiranhaOrShark(int place1, int place2, Animal[][] animalBoard) { // check if pirhana or shark cards
	...
	public static boolean isNoMoreMatchLeft(char[][] board, Animal[][] animalBoard) { 											// matches.
		...
		for (int i = 0; i < remainAnimals.length; i++) {
			for (int j = i+1; j < remainAnimals.length; j++) {// check with the remain animals if there are more possible matches
	...
	public static Animal[] checkRemainAnimals(char[][] gameBoard, Animal[][] animalBoard) {
		int count = countCovered(gameBoard);// counts how many covered left
		Animal[] animal = new Animal[count];
		int place = 0;
		for (int i = 0; i < gameBoard.length; i++) {
			for (int j = 0; j < gameBoard[0].length; j++) {
				if (gameBoard[i][j] == COVERED) {
					animal[place] = animalBoard[i][j];// creating an array with the animak cards
					place++;
				}
			}
		}
